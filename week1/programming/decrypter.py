...
for i in range(len(MSGS_E)-1):
    ...
    for j in range(i+1, len(MSGS_E)):
        aux.append(hex_xor(MSGS_E[i][:longitud],MSGS_E[j][:longitud]))
...
for i in range(len(xor_table)):
    ...
    for j in range(len(xor_table[i])):
        ...
        #aux.append(''.join([hex_xor(xor_table[i][j][z]+xor_table[i][j][z+1],hexprueba) for z in range(0,len(xor_table[i][j]),2)]))
